...
<div id="join_form">
	...
	<!-- 공지로 등록되지 않은 게시글만 -->
	<c:if test="${board.board_notice == 'N' }">
		<!-- QnA게시판  -->
			...
			<div class="order_button_area">
				<p>
					<c:if test="${board.board_writer == id || grade == 'A'.charAt(0) }">
						<a href="boardViewAction.bo?board_type=qna&board_num=${board.board_num }&pro_num=${prd.pro_num}&path=/board/board_modify_form" class="b">MODIFY</a>
		...
		<c:if test="${board.board_type=='review' }">
			...
			<div class="order_button_area">
				<p>
					<c:if test="${board.board_writer == id || grade == 'A'.charAt(0) }">
						<a href="boardViewAction.bo?board_type=review&board_num=${board.board_num }&pro_num=${prd.pro_num}&path=/board/board_modify_form" class="b">MODIFY</a>
						<a href="#" class="b" onclick="del()">DELETE</a>
					</c:if>
				</p>
			</div>
	...
	<!-- 공지로 등록된 게시글일 경우 -->
	<c:if test="${board.board_type == 'notice' || board.board_notice == 'Y'}">
		<div class="join_table">
		<table>
			<tr>
				<th>SUBJECT</th>
		...
		<div class="order_button_area">
			<p>
				<c:if test="${grade == 'A'.charAt(0) }">
					<a href="boardViewAction.bo?board_type=notice&board_num=${board.board_num }&pro_num=${prd.pro_num}&path=/board/board_modify_form" class="b">MODIFY</a>
					<a href="#" class="b" onclick="del()">DELETE</a>
				</c:if>
			</p>
		</div>
	...
	<c:if test="${board.board_step == 'Y' || board.board_type == 'answer' }">
		...
		<c:if test="${board.board_type == 'answer' }">
			<div class="order_button_area">
				<p>
					<c:if test="${board.board_writer == id || grade == 'A'.charAt(0) }">
						<a href="boardViewAction.bo?board_type=answer&board_num=${board.board_num }&pro_num=${prd.pro_num}&path=board_modify_form" class="b">MODIFY</a>
						<a href="#" class="b" onclick="del()">DELETE</a>
					</c:if>
				</p>
			</div>
